...
const ProductSearch = (props) => {
    ...
    const filterProducts = isAllProducts && values.warehouse_id ? products && Array.isArray(products) ? products.map((item) => ({
        name: item.attributes.name, code: item.attributes.code, id: item.id
    })) : [] : values.warehouse_id && products && Array.isArray(products) ? products.filter((qty) => qty && qty.attributes && qty.attributes.stock && qty.attributes.stock.quantity > 0).map((item) => ({
        name: item.attributes.name, code: item.attributes.code, id: item.id
    })) : []
    ...
    const onProductSearch = (code) => {
        if (!values.warehouse_id) {
            ...
            const newId = products && Array.isArray(products) ? products.filter((item) => item.attributes.code === code || item.attributes.code === code.code).map((item) => item.id) : [];
            const finalIdArrays = customProducts && Array.isArray(customProducts) ? customProducts.map((id) => id.product_id) : [];
