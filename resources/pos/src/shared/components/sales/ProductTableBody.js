...
const ProductTableBody = (props) => {
    ...
    useEffect(() => {
        const fetchLastPrice = async () => {
            // Check if customerId exists and is not empty string
            let currentCustomerId = null;
            if (customerId) {
                if (typeof customerId === 'object' && customerId.value) {
                    currentCustomerId = customerId.value;
                } else if (typeof customerId === 'string' && customerId !== '') {
            ...
            if (singleProduct && singleProduct.id && currentCustomerId) {
                try {
                    const response = await apiConfig.get(`get-last-sale-price/${singleProduct.id}/${currentCustomerId}`);
                    if (response.data.success && response.data.data !== null) {
                        setLastPrice(currencySymbolHandling(allConfigData, frontSetting.value && frontSetting.value.currency_symbol, response.data.data.toFixed(2)));
                    } else {
                        setLastPrice('-');
                    }
                } catch (error) {
